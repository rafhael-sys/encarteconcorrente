#!/usr/bin/env python3
"""Ingestao da janela de 2026-08-19 (analise diaria do Claude).

Adiciona 17 acoes NOVAS (classificacao/dedup ja feita nesta sessao). Descartes:
  - DcMLkbbFFFz  (teaser "Amanha e Quarta e Quinta Verde" Favorito, sem precos)
  - DcMNOZdzMHP  (sorteio "Numero da Sorte" Favorito, sem precos)
  - DcLqcdgzFlQ  (sorteio "Bilhete Dourado" Ster Bom, sem precos)
  - DcNvN6_Frey  (Nordestao "Oferta Surpresa do Dia" app, produto borrado, sem preco)
  - DcMYjH2oDZh  (Santo Antonio: subconjunto do encarte Db4BsEcoMR0, mesmo periodo 11-19,
                  todos os 16 produtos ja capturados aos mesmos precos -> 0 novo)

canon: canonicalizacao por nrm_tokens (mesma logica de ingest_20260817.py);
...
DRY_RUN por padrao. Rode 'python3 ingest_20260819.py commit' p/ gravar.
"""
import json
import os
import re
...
def main() -> None:
    """Ingesta acoes/produtos/canon da janela 2026-08-19."""
    ...
    for spec in novo["actions"]:
        ...
        act = {
            ...
            "shortcode": sid,
            # caption original da fila (dado nao confiavel, so exibicao); fallback ao spec
