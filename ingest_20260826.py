#!/usr/bin/env python3
"""Ingestao da janela de 2026-08-26 (analise diaria do Claude).

Adiciona acoes NOVAS (classificacao/dedup ja feita nesta sessao). Descartes e
decisoes de dedup ficam documentados em _new_data_20260826.json ("descartes").

canon: canonicalizacao por nrm_tokens (mesma logica de ingest_20260825.py);
NUNCA une pares marcados DIFERENTES em regras_similaridade.md.

Dados extraidos: _new_data_20260826.json (actions + products). Caption
preservada da fila. DRY_RUN por padrao; rode
'python3 ingest_20260826.py commit' para gravar.
"""
import json
import os
import re
...
def main() -> None:
    """Ingesta acoes/produtos/canon da janela 2026-08-26."""
    ...
    novo = json.load(open(path("_new_data_20260826.json"), encoding="utf-8"))
