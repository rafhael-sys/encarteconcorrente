#!/usr/bin/env python3
# Ingestao da janela 10/07/2026 — Instagram.
# Aprovados (6):
#   Favorito (Dal8u9zjMyI)  Reinauguracoes Festa em Dobro   10-16/07 (impresso)
#   Mar Vermelho (Dal9yOEkQUV) Festival da Casa             10/07-09/08 (impresso)
#   Leva Mais JC (Daly8x-gDJ1) Feirao do Acougue            10-12/07 (impresso)
#   Santo Antonio (Dal5zw8gq88) Fim de Semana               10-12/07 (impresso)
#   Santo Antonio (Dai-VTgHzTC) Ofertas gerais              09-15/07 (impresso)
#   Santo Antonio (Dag4P7BA5hC) Quarta e Quinta Verde       08-09/07 (impresso)
# Descartados (2): Damt7F5m76N (receita de pizza, sem precos),
#                  Dal8IOFEY5P (teaser Festival da Casa, sem precos).
# Sem regra de perfil aplicavel. Nenhuma duplicata: os periodos/banners novos
# nao coincidem com nenhuma acao ja em actions.json (Santo Antonio nao tinha
# nenhuma acao; demais periodos sao inéditos para o banner).
...
newprod  = json.load(open(path("data/_ingest_20260710_produtos.json"), encoding="utf-8"))
byshort  = {p["shortcode"]: p for p in fila}

for f in ("data/actions.json","data/products.json","data/canon.json"):
    shutil.copy(path(f), path(f + ".bak-20260710-ingest"))
...
# metadados por acao (inicio/fim/titulo vindos do que esta IMPRESSO nas paginas)
META = {
    "Dal8u9zjMyI": ("2026-07-10", "2026-07-16", "Festa em Dobro — Reinaugurações Zona Norte e Macaíba (10 a 16/07)"),
    "Dal9yOEkQUV": ("2026-07-10", "2026-08-09", "Festival da Casa (10/07 a 09/08)"),
    "Daly8x-gDJ1": ("2026-07-10", "2026-07-12", "Feirão do Açougue (10 a 12/07)"),
    "Dal5zw8gq88": ("2026-07-10", "2026-07-12", "Ofertas do Fim de Semana (10 a 12/07)"),
    "Dai-VTgHzTC": ("2026-07-09", "2026-07-15", "Ofertas Santo Antônio (09 a 15/07)"),
    "Dag4P7BA5hC": ("2026-07-08", "2026-07-09", "Quarta e Quinta Verde — Hortifrúti (08 e 09/07)"),
}
NOVAS = list(META.keys())
...
for sc in NOVAS:
    ...
    act = {
        "id": sc, "perfil": src["perfil"], "titulo": titulo,
