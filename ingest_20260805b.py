#!/usr/bin/env python3
"""Ingestao da 2a janela de 2026-08-05 (janela da NOITE).

Consome data/_extract/w0805e_*.json (produtos por pagina, extraidos por visao
nesta rodada) + data/_extract/w0805e_meta.json (datas/titulo/discard por
shortcode, montado a partir do retorno dos extratores). Mesmas regras do 0805:
- Post sem preco / teaser / B2B / fora-RN -> DESCARTADO (batch vazio cai fora).
...
- STORY cujo id JA existe em actions.json -> ESTENDE a acao (adiciona os frames
  novos com produto + seus produtos), em vez de pular. Nao reextrai frame ja
  presente em products.json.
- canon: canonicalizacao por nrm_tokens; NUNCA une pares DIFERENTES.

DRY_RUN por padrao. Rode 'python3 ingest_20260805b.py commit' p/ gravar.
"""
import json
import os
import re
...
# Overrides manuais (preenchidos apos ler o dry-run):
# DbrRhX7nyEl: Feirao Hortifruti MV (06-07) e oferta NOVA distinta; nenhuma acao
# existente tem esse periodo. O overlap 0.72 e falso-positivo (nomes genericos de
# hortifruti) e o story MV que a repetia foi descartado -> este feed e a unica
# captura desses precos. Mantido.
KEEP_OVERRIDE = {"DbrRhX7nyEl"}   # forca manter mesmo com overlap alto
DISCARD_OVERRIDE = set()   # forca descartar (re-post identico da mesma rodada, etc)
...
META = json.load(open(path("data/_extract/w0805e_meta.json"), encoding="utf-8"))
...
def eff_id(sc):
    # id_override: p/ story mal-rotulado (ex.: coleta 'miramar' que na verdade e
    # Miramar, mas o id ja foi ocupado por conteudo Mirassol numa janela anterior)
...
for sc in order:
    ...
    eid = eff_id(sc)
    if eid in byid:
        # STORY extend: adiciona frames novos (com produto) a acao existente
        a = byid[eid]
...
# ---------- relatorio ----------
print("=" * 72)
print("DRY_RUN" if DRY_RUN else "COMMIT", "- ingest_20260805b (janela noite)")
print("=" * 72)
...
if DRY_RUN:
    print("\n(dry-run: nada gravado. rode 'python3 ingest_20260805b.py commit')")
    sys.exit(0)

# ================= COMMIT =================
for f in ("data/actions.json", "data/products.json", "data/canon.json"):
    shutil.copy(path(f), path(f + ".bak-20260805b"))
