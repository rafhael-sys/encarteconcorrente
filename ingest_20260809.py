#!/usr/bin/env python3
"""Ingestao da janela de 2026-08-09.

Consome data/_extract/w0809_*.json (formato RICO por post, escrito pelos
subagentes de visao): cada arquivo tem
  {shortcode, discard, discard_reason, inicio, fim, titulo, pages:{pagekey:[...]}}
Arquivos com sufixo __A/__B pertencem ao MESMO shortcode (story dividido em
partes) e sao MESCLADOS (pages unidas; datas nao-nulas preferidas).

Mesmas regras das janelas anteriores (ver ingest_20260808.py):
- Post sem preco / teaser / B2B / institucional -> DESCARTADO.
...
DRY_RUN por padrao. Rode 'python3 ingest_20260809.py commit' p/ gravar.
"""
import json
import os
import re
...
# story do Favorito = compilado do flyer Favoritaço + cards "Faz o Pix" ja
# capturados nos feeds/stories da semana (overlap 0.59 sem o A1a; com o A1a, que
# e a pagina do flyer Favoritaço, passaria de 0.6). Subconjunto -> descartar.
DISCARD_OVERRIDE = {"story_favoritosuper_20260809"}
...
for fn in sorted(os.listdir(edir)):
    ...
    if not isinstance(d, dict) or "pages" not in d:
        print("[aviso] formato inesperado:", fn)
        continue
    sc = d.get("shortcode")
    if not sc:
        print("[aviso] sem shortcode:", fn)
        continue
    pb = post_batch.setdefault(sc, {})
    for k, v in (d.get("pages") or {}).items():
        pb[k] = v
    m = META.setdefault(sc, {"inicio": None, "fim": None, "titulo": "",
                             "discard": False, "reason": ""})
    if d.get("inicio") and not m["inicio"]:
        m["inicio"] = d["inicio"]
    if d.get("fim") and not m["fim"]:
        m["fim"] = d["fim"]
    if d.get("titulo") and not m["titulo"]:
        m["titulo"] = d["titulo"]
    if d.get("discard"):
        m["discard"] = True
        m["reason"] = d.get("discard_reason", "")
...
# ---------- relatorio ----------
print("=" * 72)
print("DRY_RUN" if DRY_RUN else "COMMIT", "- ingest_20260809")
print("=" * 72)
...
if DRY_RUN:
    print("\n(dry-run: nada gravado. rode 'python3 ingest_20260809.py commit')")
    sys.exit(0)

# ================= COMMIT =================
for f in ("data/actions.json", "data/products.json", "data/canon.json"):
    shutil.copy(path(f), path(f + ".bak-20260809"))
