#!/usr/bin/env python3
"""Ingestao da janela de 2026-08-06 (2a rodada / noite).

Consome data/_extract/w0806b_*.json (produtos por pagina, extraidos por visao)
+ data/_extract/w0806b_meta.json (datas/titulo/discard por shortcode).
...
DRY_RUN por padrao. Rode 'python3 ingest_20260806b.py commit' p/ gravar.
"""
import json
import os
import re
...
META = json.load(open(path("data/_extract/w0806b_meta.json"), encoding="utf-8"))
...
# ---------- relatorio ----------
print("=" * 72)
print("DRY_RUN" if DRY_RUN else "COMMIT", "- ingest_20260806b")
print("=" * 72)
...
if DRY_RUN:
    print("\n(dry-run: nada gravado. rode 'python3 ingest_20260806b.py commit')")
    sys.exit(0)

# ================= COMMIT =================
for f in ("data/actions.json", "data/products.json", "data/canon.json"):
    shutil.copy(path(f), path(f + ".bak-20260806b"))
