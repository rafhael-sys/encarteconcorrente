#!/usr/bin/env python3
"""Ingestao da janela de 2026-08-12.

Consome data/_extract/w0812_*.json (formato RICO por post): cada arquivo tem
  {shortcode, discard, discard_reason, inicio, fim, titulo, pages:{pagekey:[...]}}

Mesmas regras das janelas anteriores (ver ingest_20260811.py):
...
DRY_RUN por padrao. Rode 'python3 ingest_20260812.py commit' p/ gravar.
"""
import json
import os
import re
...
# --- carrega extracao RICA (w0812_*.json) ---
...
# ---------- relatorio ----------
print("=" * 72)
print("DRY_RUN" if DRY_RUN else "COMMIT", "- ingest_20260812")
print("=" * 72)
...
if DRY_RUN:
    print("\n(dry-run: nada gravado. rode 'python3 ingest_20260812.py commit')")
    sys.exit(0)

# ================= COMMIT =================
for f in ("data/actions.json", "data/products.json", "data/canon.json"):
    shutil.copy(path(f), path(f + ".bak-20260812"))
